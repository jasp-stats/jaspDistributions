context("Discover Distributions - Gompertz")

options <- jaspTools::analysisOptions("LDgompertz")
...
set.seed(1)
results <- jaspTools::runAnalysis("LDgompertz",
                                  data.frame(gompertz = jaspDistributions:::rgompertz(n = 100, shape = options[["shape"]], scale = options[["scale"]])),
                                  options)
...
test_that("Empirical Cumulative Distribution plot matches", {
  ...
  jaspTools::expect_equal_plots(testPlot, "empirical-cumulative-distribution", dir="LDgompertz")
})

test_that("Histogram plot matches", {
  plotName <- results[["results"]][["dataContainer"]][["collection"]][["dataContainer_histogram"]][["data"]]
  testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
  jaspTools::expect_equal_plots(testPlot, "histogram", dir="LDgompertz")
})
...
test_that("Observed Moments table results match", {
  ...
  jaspTools::expect_equal_tables(table,
                                 list(0.603042020959476, 1, 0.603042020959476, 0.137668922378622, 2,
                                      0.501328601421512))
})
...
test_that("Descriptives table results match", {
  ...
  jaspTools::expect_equal_tables(table,
                                 list(1.76072345431243, 0.603042020959476, 0.512276236848848, 0.0133907346774114,
                                      0.329454741772288, 0.899168940031794, 100, 0.372906848360504,
                                      0.139059517554164, "gompertz"))
})

test_that("Estimated Parameters table results match", {
  table <- results[["results"]][["mleContainer"]][["collection"]][["mleContainer_estParametersTable"]][["data"]]
  jaspTools::expect_equal_tables(table,
                                 list(0.517133067960924, 0.169065468140162, "<unicode>", 0.177588773348018,
                                      0.865200667781686, 1.50414234851941, 0.996277484877358, "b",
                                      0.259119487729377, 2.01200721216145))
})
...
test_that("Empirical vs. Theoretical CDF plot matches", {
  ...
  testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
  jaspTools::expect_equal_plots(testPlot, "empirical-vs-theoretical-cdf", dir="LDgompertz")
})
...
test_that("Histogram vs. Theoretical PDF plot matches", {
  ...
  testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
  jaspTools::expect_equal_plots(testPlot, "histogram-vs-theoretical-pdf", dir="LDgompertz")
})

test_that("Fit Statistics table results match", {
  table <- results[["results"]][["mleContainer"]][["collection"]][["mleContainer_mleFitAssessment"]][["collection"]][["mleContainer_mleFitAssessment_fitStatisticsTable"]][["data"]]
  jaspTools::expect_equal_tables(table,
                                 list(0.890388973901891, 0.0579349213271444, "Kolmogorov-Smirnov", 0.931589650147336,
                                      0.0404719207842354, "Cram<unicode>r-von Mises", 0.969999552531264,
                                      0.250647657136682, "Anderson-Darling"))
})
...
test_that("P-P plot matches", {
  ...
  testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
  jaspTools::expect_equal_plots(testPlot, "p-p-plot", dir="LDgompertz")
})
