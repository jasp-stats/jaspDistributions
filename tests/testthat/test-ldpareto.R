context("Discover Distributions - Pareto")

options <- analysisOptions("LDpareto")
...
options$variable <- "pareto"
set.seed(1)
results <- jaspTools::runAnalysis("LDpareto",
                                  data.frame(pareto = jaspDistributions:::rpareto(n = 100, options[['shape']], options[['scale']])),
                                  options)

test_that("Descriptives table results match", {
  table <- results[["results"]][["dataContainer"]][["collection"]][["dataContainer_summary"]][["data"]]
  jaspTools::expect_equal_tables(table,
                                 list(123.549754915117, 4.68242475780387, 1.95264762208588, 1.01357206766661,
                                      1.47733340868051, 4.29559676333304, 100, 12.5707493645755, 158.023739586976,
                                      "pareto"))
})
...
test_that("Estimated Parameters table results match", {
  ...
  jaspTools::expect_equal_tables(table,
                                 list(1.04980133719044, "<unicode>", 1.01357206766661, "<unicode>"
                                 ))
})
...
test_that("Empirical vs. Theoretical CDF plot matches", {
  ...
  jaspTools::expect_equal_plots(testPlot, "empirical-vs-theoretical-cdf", dir="LDpareto")
})
...
test_that("Histogram vs. Theoretical PDF plot matches", {
  ...
  testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
  jaspTools::expect_equal_plots(testPlot, "histogram-vs-theoretical-pdf", dir="LDpareto")
})

test_that("Fit Statistics table results match", {
  table <- results[["results"]][["mleContainer"]][["collection"]][["mleContainer_mleFitAssessment"]][["collection"]][["mleContainer_mleFitAssessment_fitStatisticsTable"]][["data"]]
  jaspTools::expect_equal_tables(table,
                                 list(0.551750506977832, 0.0795335244498101, "Kolmogorov-Smirnov", 0,
                                      0, 0, 0.71891899449626, 0.318238840771031, "Cram<unicode>r-von Mises",
                                      0, 0, 0, 0.000599838025738153, "<unicode>", "Anderson-Darling"
                                 ))
})
...
test_that("P-P plot matches", {
  ...
  testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
  jaspTools::expect_equal_plots(testPlot, "p-p-plot", dir="LDpareto")
})
